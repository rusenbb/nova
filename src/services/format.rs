//! Shared formatting utilities for services
...
pub fn format_number(value: f64) -> String {
    if value.fract() == 0.0 && value.abs() < 1e12 {
        ...
        // Limit to 10 decimal places and trim trailing zeros
...
mod tests {
    ...
    fn test_format_precision() {
        // Should truncate very long decimals
        let result = format_number(1.0 / 3.0);
        assert!(result.len() <= 12); // "0.3333333333" max
    }
}
